                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {event.facultyCoordinator && (
                                            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-1">Faculty Coordinator</p>
                                                <p className="font-extrabold text-slate-800">{event.facultyCoordinator.username}</p>
                                                <p className="text-sm text-slate-500 mt-1">{event.facultyCoordinator.email}</p>
                                                {event.facultyCoordinator.phone && (
                                                    <p className="text-sm text-slate-500 mt-0.5">Ph: {event.facultyCoordinator.phone}</p>
                                                )}
                                            </div>
                                        )}
                                        {event.studentCoordinator && (
                                            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-1">Student Coordinator</p>
                                                <p className="font-extrabold text-slate-800">{event.studentCoordinator.username}</p>
                                                <p className="text-sm text-slate-500 mt-1">{event.studentCoordinator.email}</p>
                                                {event.studentCoordinator.phone && (
                                                    <p className="text-sm text-slate-500 mt-0.5">Ph: {event.studentCoordinator.phone}</p>
                                                )}
                                            </div>
                                        )}
                                    </div>
                        ...
                        {!showRegForm && (
                            <div className="space-y-6">
                                ...
                                {['Class Coordinator', 'Program Coordinator', 'Admin', 'Faculty', 'Faculty Coordinator'].includes(user?.role) ? (
                                    <div className="p-4 bg-blue-50 text-blue-700 border border-blue-100 rounded-2xl text-center text-sm font-bold">
                        ...
                        {user?.role === 'Association Member' && !showRegForm &&
                         ...
                         (settings.volunteerRestriction === 'all' || new Date() < new Date(event.eventDate)) && (
                            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mt-6">
                                ...
                                {volunteerApplied ? (
                                    <div className="p-4 bg-emerald-50 text-emerald-700 rounded-2xl text-center text-sm font-bold border border-emerald-100">
                                        ✓ Application Submitted — Awaiting Admin Approval
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        <textarea
                                            className="w-full border border-gray-200 rounded-2xl p-4 text-sm font-medium text-gray-700 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-200 h-24"
