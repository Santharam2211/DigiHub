                                ...
                                <div className="lg:col-span-3 p-12 space-y-8">
                                    ...
                                    <div className="space-y-4">
                                        {event.displayWinners.map((winner, idx) => (
                                            <div 
                                                key={winner._id}
                                                className={`relative overflow-hidden group p-6 rounded-3xl border transition-all duration-300 flex items-center justify-between ${
                                                    parseInt(winner.position) === 1 ? 'bg-gradient-to-r from-yellow-50 to-amber-50 border-yellow-200 hover:shadow-xl hover:shadow-yellow-100/50' :
                                                    parseInt(winner.position) === 2 ? 'bg-gradient-to-r from-slate-50 to-gray-100 border-slate-200 hover:shadow-xl hover:shadow-slate-200/50' :
                                                    parseInt(winner.position) === 3 ? 'bg-gradient-to-r from-orange-50 to-amber-50 border-orange-200 hover:shadow-xl hover:shadow-orange-100/50' :
                                                    'bg-slate-50 border-transparent hover:border-indigo-100 hover:bg-white hover:shadow-xl hover:shadow-indigo-100/50'
                                                }`}
                                            >
                                                <div className={`absolute -right-10 -top-10 w-40 h-40 rounded-full blur-3xl opacity-20 transition-opacity group-hover:opacity-40 ${
                                                    parseInt(winner.position) === 1 ? 'bg-yellow-400' :
                                                    parseInt(winner.position) === 2 ? 'bg-slate-400' :
                                                    parseInt(winner.position) === 3 ? 'bg-orange-500' :
                                                    'bg-indigo-400'
                                                }`}></div>
                                                <div className="flex items-center gap-6">
                                                    <div className="relative z-10">
                                                        {winner.participant?.profileImage ? (
                                                            <div className="relative">
                                                                ...
                                                                />
                                                                <div className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-full flex items-center justify-center text-white font-black text-sm shadow-md border-2 border-white ${
                                                                    parseInt(winner.position) === 1 ? 'bg-gradient-to-br from-yellow-400 to-orange-500' :
                                                                    parseInt(winner.position) === 2 ? 'bg-gradient-to-br from-slate-400 to-slate-500' :
                                                                    parseInt(winner.position) === 3 ? 'bg-gradient-to-br from-orange-400 to-amber-600' :
                                                    ...
                                                    <div className="relative z-10">
                                                        ...
                                                        <p className="text-indigo-600 font-bold text-sm tracking-wide uppercase mt-1">{winner.prize}</p>
                                                    </div>
                                                </div>
