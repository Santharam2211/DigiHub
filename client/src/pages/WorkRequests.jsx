    ...
    return (
        <div className="space-y-8 max-w-6xl mx-auto">
            ...
            <div className="flex justify-between items-center bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
                <div>
                    <h1 className="text-3xl font-black text-slate-900">Work Requests</h1>
                    <p className="text-slate-500 mt-2 font-medium">Manage and track task assignments</p>
                </div>
                {canCreate && (
                    <button 
            ...
            {requests.length === 0 ? (
                <div className="bg-white p-12 rounded-3xl border border-slate-100 text-center shadow-sm">
                    ...
                    <p className="text-slate-500 mt-2">There are currently no work requests to display.</p>
                </div>
            ) : (
                <div className="grid gap-6">
                    {requests.map(req => {
                        ...
                        return (
                            <div key={req._id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-6 hover:shadow-md transition-all">
                                <div className="flex-1 space-y-4">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <div className="flex items-center gap-3 mb-2">
                                                <h3 className="text-xl font-extrabold text-slate-900">{req.title}</h3>
                                            ...
                                            {req.status === 'Completed' && (req.workDetails || req.experienceFeedback) && user.role === 'Admin' && (
                                                <div className="mt-4 p-4 bg-emerald-50 rounded-xl border border-emerald-100 space-y-3">
                                                    <h4 className="text-xs font-black text-emerald-700 uppercase tracking-wider">Completion Feedback</h4>
                                                    {req.workDetails && (
                                                        <div>
                                                            <span className="block text-xs font-bold text-slate-500 mb-1">Work Details</span>
                                                            <p className="text-sm text-slate-700 bg-white p-3 rounded-lg border border-emerald-100">{req.workDetails}</p>
                                                        </div>
                                                    )}
                                                    {req.experienceFeedback && (
                                                        <div>
                                                            <span className="block text-xs font-bold text-slate-500 mb-1">Experience</span>
                                                            <p className="text-sm text-slate-700 bg-white p-3 rounded-lg border border-emerald-100">{req.experienceFeedback}</p>
                                                        </div>
                                                    )}
                                                </div>
                                            )}
                                    ...
                                    <div className="flex flex-wrap gap-4 text-sm font-medium text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                ...
                                <div className="flex flex-col gap-3 min-w-[200px] border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 justify-center">
                                    {/* Admin Actions */}
                                        ...
                                        <div className="flex flex-col gap-2 mt-auto">
                                            {isAssignee && req.status === 'Assigned' && (
                                                <button 
                                                    ...
                                                    className="w-full py-2 px-3 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-xl font-bold text-sm hover:bg-indigo-600 hover:text-white transition-all text-center"
                                                >
                                                    Accept Request
                                                </button>
                                            )}
                                            {(isAssignee || user.role === 'Admin') && req.status === 'Accepted' && (
                                                <button 
                                                    ...
                                                    className="w-full py-2 px-3 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl font-bold text-sm hover:bg-emerald-600 hover:text-white transition-all text-center"
                                                >
                                                    Mark Done
                                                </button>
                                            )}
                                            {(isRequester || user.role === 'Admin') && (req.status === 'Pending' || req.status === 'Assigned') && (
                                                <button 
                                                    ...
                                                    className="flex-1 py-2 px-3 bg-red-50 text-red-700 border border-red-200 rounded-xl font-bold text-sm hover:bg-red-600 hover:text-white transition-all text-center"
                                                >
                                                    Cancel
                                                </button>
                                            )}
                ...
                {showCreateModal && (
                    ...
                    >
                        <motion.div 
                            ...
                            className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-xl border border-slate-100"
                        >
                            <h2 className="text-2xl font-black text-slate-900 mb-6">New Work Request</h2>
                ...
                {showFeedbackModal && (
                    ...
                    >
                        <motion.div 
                            ...
                            className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-xl border border-slate-100"
                        >
                            <h2 className="text-2xl font-black text-slate-900 mb-6">Complete Work Request</h2>
                            <form onSubmit={handleCompleteSubmit} className="space-y-5">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">Work Details</label>
                                    <textarea 
                                        required
                                        value={workDetails}
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">Experience Feedback</label>
                                    <textarea 
                                        required
