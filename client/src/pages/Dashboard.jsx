...
const FacultyDashboard = ({ user }) => {
    ...
    const fetchFacultyData = async () => {
        try {
            const inchargeRes = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/events/my-incharge`);
    ...
    const downloadAttendance = async (eventId) => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/attendance/export/${eventId}`, {
                responseType: 'blob',
                headers: {
    ...
    const downloadFeedback = async (eventId) => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/feedback/export/excel/${eventId}`, {
                responseType: 'blob',
                headers: {
                    Authorization: `Bearer ${user.token}`
                }
    ...
    const downloadAttendancePDF = async (eventId) => {
        try {
            const res = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/attendance/export-pdf/${eventId}`, {}, {
                responseType: 'blob',
                headers: {
                    Authorization: `Bearer ${user.token}`
                }
    ...
    const downloadFeedbackPDF = async (eventId) => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/feedback/export/pdf/${eventId}`, {
                responseType: 'blob',
                headers: {
                    Authorization: `Bearer ${user.token}`
                }
    ...
    const downloadRegistrationPDF = async (eventId) => {
        try {
            const res = await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/registrations/export/pdf/${eventId}`, {}, {
                responseType: 'blob',
                headers: {
                    Authorization: `Bearer ${user.token}`
                }
        ...
        <div className="space-y-12 pb-40">
            ...
            <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
                <div className="flex border-b border-slate-100 bg-slate-50/50">
                    <div className="flex-1 py-5 text-sm font-black uppercase tracking-widest text-indigo-600 bg-white text-center relative">
                        My Events (Incharge)
                ...
                <div className="p-8">
                    <div className="space-y-6">
                        <h2 className="text-2xl font-black text-slate-900">Events I'm In-Charge Of</h2>
                        {inchargeEvents.length === 0 ? (
                            ...
                            <div className="grid gap-5">
                                {inchargeEvents.map(ev => {
                                    ...
                                    return (
                                        <div key={ev._id} className="p-6 bg-indigo-50 border border-indigo-100 rounded-2xl hover:bg-indigo-100 transition-colors group">
                                            <div className="flex items-center gap-6 cursor-pointer" onClick={() => goToEvent(ev._id)}>
                                                <div className="w-16 h-16 rounded-2xl bg-white flex items-center justify-center shadow-sm border border-indigo-100 shrink-0">
                                                    <Calendar className="w-7 h-7 text-indigo-500" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <h3 className="font-extrabold text-slate-900 text-lg truncate">{ev.title}</h3>
                                                    <div className="flex flex-wrap gap-4 mt-1 text-sm text-slate-500 font-bold">
                                            ...
                                            <div className="mt-5 pt-5 border-t border-indigo-100/50 grid grid-cols-3 gap-3">
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <button onClick={() => downloadAttendance(ev._id)} className="flex-1 py-2 bg-white text-indigo-600 rounded-xl font-bold text-xs shadow-sm border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center gap-2">
                                                            <Download className="w-3.5 h-3.5" /> Excel
                                                        </button>
                                                        <button onClick={() => downloadAttendancePDF(ev._id)} className="flex-1 py-2 bg-white text-indigo-600 rounded-xl font-bold text-xs shadow-sm border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center gap-2">
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <button onClick={() => downloadFeedback(ev._id)} className="flex-1 py-2 bg-white text-emerald-600 rounded-xl font-bold text-xs shadow-sm border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center gap-2">
                                                            <Download className="w-3.5 h-3.5" /> Excel
                                                        </button>
                                                        <button onClick={() => downloadFeedbackPDF(ev._id)} className="flex-1 py-2 bg-white text-emerald-600 rounded-xl font-bold text-xs shadow-sm border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center gap-2">
                                                            <Download className="w-3.5 h-3.5" /> PDF
                                                        </button>
                                                    </div>
                                                </div>
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <button onClick={() => downloadRegistrationPDF(ev._id)} className="flex-1 py-2 bg-white text-amber-600 rounded-xl font-bold text-xs shadow-sm border border-amber-100 hover:bg-amber-600 hover:text-white transition-all flex items-center justify-center gap-2">
                                                            <Download className="w-3.5 h-3.5" /> PDF
                                                        </button>
                                                    </div>
                                                </div>
...
const Dashboard = () => {
    ...
    const fetchAssociationDetails = async () => {
        try {
            const [transRes, eventsRes, profileRes] = await Promise.all([
                axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/transactions/my`),
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const requests = [
                    axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/registrations/my`),
                    axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/nominations`)
                ];
                // Also fetch incharge events for Association Members
                if (user?.role === 'Association Member') {
                    requests.push(axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/events/my-incharge`));
                }
    ...
    const handleSpendSubmit = async (e) => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/transactions/spend`, formData, {
    ...
    const handleReimburseSubmit = async (e) => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/transactions/reimburse`, data);
    ...
    const handleDownloadCertificate = async (regId) => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/certificates/download/${regId}`, {
                responseType: 'blob'
            });
        ...
        <div className="space-y-12 pb-40">
                ...
                {isBirthday && (
                    ...
                    >
                        ...
                        <div className="relative z-10 flex items-center gap-6">
                            <div className="w-20 h-20 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-sm"><Cake className="w-10 h-10" /></div>
                            <div>
            ...
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-wrap justify-between items-center gap-8">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h1 className="text-4xl font-black text-slate-900 tracking-tight">Welcome, {user.username}.</h1>
                        <p className="text-slate-500 font-medium mt-1">Here's what's happening on your journey.</p>
                    </div>
                </div>
            ...
            <div className="grid lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-2 space-y-12">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid gap-5">
                                {inchargeEvents.map(ev => {
                                    const isFC = ev.facultyCoordinator?._id === user._id || ev.facultyCoordinator === user._id;
                                    return (
                                        <a key={ev._id} href={`/events/${ev._id}`} className="flex flex-col gap-6 p-6 bg-indigo-50 border border-indigo-100 rounded-[2.5rem] hover:bg-indigo-100 transition-colors">
                                            <div className="flex items-center gap-6 w-full">
                                                ...
                                                <div className="flex flex-col items-end gap-2 shrink-0">
                                                    <span className={`px-3 py-1.5 rounded-full text-xs font-black border ${ev.status === 'Open' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                                        ev.status === 'Completed' ? 'bg-slate-100 text-slate-600 border-slate-200' :
                                                            'bg-amber-50 text-amber-700 border-amber-100'
                                                        }`}>{ev.status}</span>
                                                    <span className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-[10px] font-black uppercase tracking-wider">
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <button
                                                            ...
                                                            className="flex-1 py-2 bg-white text-indigo-600 rounded-xl font-bold text-xs shadow-sm border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center gap-2"
                                                        >
                                                            <Download className="w-3.5 h-3.5" /> Excel
                                                        </button>
                                                        <button
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <button
                                                            ...
                                                            className="flex-1 py-2 bg-white text-emerald-600 rounded-xl font-bold text-xs shadow-sm border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center gap-2"
                                                        >
                                                            <Download className="w-3.5 h-3.5" /> Excel
                                                        </button>
                                                        <button
                                                ...
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        <button
                                                            ...
                                                            className="flex-1 py-2 bg-white text-amber-600 rounded-xl font-bold text-xs shadow-sm border border-amber-100 hover:bg-amber-600 hover:text-white transition-all flex items-center justify-center gap-2"
                                                        >
                                                            <Download className="w-3.5 h-3.5" /> PDF
                                                        </button>
                                                    </div>
                    ...
                    {user?.role === 'Association Member' && (
                        <div className="space-y-6">
                            <div className="flex items-center justify-between">
                                <h2 className="text-2xl font-black text-slate-900">Expenses & Reimbursements</h2>
                            ...
                            <div className="bg-gradient-to-br from-indigo-900 to-slate-900 p-8 rounded-[2.5rem] text-white flex flex-wrap justify-between items-center gap-6 shadow-xl relative overflow-hidden">
                                ...
                                <div className="flex gap-4 relative z-10">
                                    <button
                                        ...
                                        className="py-4 px-6 bg-white text-indigo-900 hover:bg-slate-100 font-black rounded-2xl flex items-center gap-2 transition-all text-sm shadow-md"
                                    >
                            ...
                            <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm">
                                <h3 className="font-extrabold text-slate-900 text-lg mb-6">Recent Ledger Entries</h3>
                                            ...
                                            <tbody>
                                                {transactions.map(t => (
                                                    <tr key={t._id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                                                        <td className="py-4 text-sm font-medium text-slate-500">{new Date(t.date).toLocaleDateString()}</td>
                                                        <td className="py-4">
                                                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${t.type === 'Spent' ? 'bg-amber-50 text-amber-700 border border-amber-100' : 'bg-emerald-50 text-emerald-700 border border-emerald-100'
                                                                }`}>{t.type}</span>
                                                        </td>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            <h2 className="text-2xl font-black text-slate-900">My Nominations</h2>
                        ...
                        {nominations.length > 0 ? (
                            <div className="space-y-6">
                                {nominations.map(nom => (
                                    <div key={nom._id} className="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm group">
                                        <div className="flex justify-between items-start mb-8">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h3 className="font-black text-xl text-slate-900">{nom.postAppliedFor}</h3>
                                            ...
                                            <div className="text-right">
                                                ...
                                                <span className={`text-sm font-black ${nom.status === 'Approved' ? 'text-emerald-500' :
                                                    nom.status === 'Rejected' ? 'text-red-500' : 'text-amber-500'
                                                    }`}>
                                                    {nom.status}
                                                </span>
                                            </div>
                                            ...
                                            <div className="relative flex justify-between">
                                                {getStatusSteps(nom.status).map((step, idx) => (
                                                    <div key={idx} className="flex flex-col items-center gap-3 relative z-10">
                                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-4 ${step.isRejected ? 'bg-red-500 border-red-100 text-white' :
                                                            step.isCompleted ? 'bg-emerald-500 border-emerald-100 text-white' :
                                                                step.isCurrent ? 'bg-white border-primary-500 text-primary-600' :
                                                        ...
                                                        <span className={`text-[10px] font-black uppercase tracking-tighter ${step.isCurrent ? 'text-slate-900' : 'text-slate-400'
                                                            }`}>
                                                            {step.label}
                                                        </span>
                                                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between">
                            <h2 className="text-2xl font-black text-slate-900">My Registrations</h2>
                            <a href="/events" className="text-xs font-black text-primary-600 uppercase tracking-widest hover:underline flex items-center gap-1">
                        ...
                        {isLoading ? (
                            <div className="space-y-4">
                                {[1, 2].map(i => <div key={i} className="h-40 bg-slate-50 rounded-[2.5rem] animate-pulse"></div>)}
                            </div>
                            ...
                            <div className="space-y-6">
                                {registrations.map((reg) => (
                                    <motion.div
                                        key={reg._id}
                                        whileHover={{ y: -5 }}
                                        className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-wrap justify-between items-center gap-8 group transition-all hover:shadow-xl hover:shadow-slate-100"
                                    >
                                        <div className="flex gap-8 items-center">
                                            ...
                                            <div className="space-y-3">
                                                <h3 className="text-2xl font-black text-slate-900 leading-tight">{reg.event?.title || 'Unknown Event'}</h3>
                                                ...
                                                <div className="flex gap-2">
                                                    {reg.attendanceStatus ? (
                                                        ...
                                                        <span className="bg-amber-50 text-amber-600 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border border-amber-100">
                                            ...
                                            {reg.attendanceStatus && (
                                                <>
                                                    <button
                                                        ...
                                                        className={`flex-1 sm:flex-none py-3 px-6 font-black rounded-2xl flex items-center justify-center gap-2 transition-all ${(reg.event?.feedbackForm?.length > 0 && !reg.feedbackSubmitted)
                                                            ? 'bg-slate-50 text-slate-300 cursor-not-allowed border-2 border-dashed'
                                                            : 'bg-primary-600 text-white hover:bg-primary-500 shadow-lg shadow-primary-100'
                                                            }`}
                                                    >
                ...
                <div className="space-y-8">
                        ...
                        <div className="space-y-6">
                            <div className="flex justify-between items-end border-b border-white/5 pb-6">
                                <div>
                                    ...
                                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">Events Joined</p>
                                </div>
                                <div className="text-right">
                    ...
                    <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm space-y-8">
                        <h3 className="text-xl font-black text-slate-900 leading-tight">Next Generation Leadership.</h3>
                        <p className="text-slate-500 text-sm font-medium leading-relaxed">
                            Your participation counts. Build your portfolio by attending events and taking on coordinator roles.
                        </p>
                        <div className="pt-4">
                            <a href="/events" className="w-full py-4 px-6 bg-slate-50 hover:bg-slate-100 text-slate-900 font-black rounded-2xl flex items-center justify-center gap-3 transition-all border border-slate-100">
                ...
                {selectedReg && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md" onClick={() => setSelectedReg(null)}>
                        <motion.div
                            ...
                            className="bg-white p-12 rounded-[3.5rem] max-w-sm w-full text-center space-y-8 shadow-2xl relative"
                            onClick={e => e.stopPropagation()}
                        >
                            <div className="space-y-2 text-center">
                                <h3 className="text-3xl font-black text-slate-900 leading-tight">{selectedReg.event?.title || 'Event Pass'}</h3>
                                <p className="text-sm text-slate-400 font-bold uppercase tracking-widest">{selectedReg.event?.venue}</p>
                            </div>
                            <div className="bg-white p-8 rounded-[3rem] border-4 border-slate-50 inline-block shadow-lg relative">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xl font-mono font-black text-slate-900">{selectedReg.registrationId}</p>
                            </div>
                            <button
                ...
                {isSpendModalOpen && (
                    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setIsSpendModalOpen(false)}>
                        <motion.div
                            ...
                            animate={{ opacity: 1, scale: 1, y: 0 }}
                            exit={{ opacity: 0, scale: 0.9, y: 20 }}
                            className="bg-white p-8 rounded-3xl max-w-xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in fade-in zoom-in-95 duration-250"
                            onClick={e => e.stopPropagation()}
                        >
                            ...
                            <form onSubmit={handleSpendSubmit} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center justify-center w-full">
                                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-250 border-dashed rounded-2xl cursor-pointer bg-slate-50 hover:bg-slate-100/50 transition-all">
                                            <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                                <Upload className="w-8 h-8 text-slate-400 mb-2" />
                                                <p className="text-sm font-bold text-slate-500">Click to upload proof file</p>
