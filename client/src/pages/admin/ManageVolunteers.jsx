...
const ManageVolunteers = () => {
    ...
    const fetchApplications = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/volunteers/all`);
    ...
    const handleUpdate = async (id, updates) => {
        setUpdatingId(id);
        try {
            await axios.put(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/volunteers/${id}`, updates);
    ...
    return (
        <div className="space-y-12 pb-40">
            ...
            {/* Filters */}
            <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm flex flex-wrap gap-4 items-center">
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-[3rem] border-2 border-dashed border-slate-100 py-24 text-center space-y-4">
                            ...
                            <div className="flex flex-wrap items-start gap-6">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap items-center gap-3 mb-2">
                                        <h3 className="font-black text-xl text-slate-900">{app.applicant?.username}</h3>
                                        <span className={`px-3 py-1 rounded-full text-xs font-black border ${statusColors[app.status]}`}>
                                            {app.status}
                                        </span>
                                        {app.onDutyIssued && (
                                            <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 text-xs font-black">
                                                <Award className="w-3 h-3" /> On-Duty Issued
                                            </span>
                                        )}
                                    </div>
                                    <div className="flex flex-wrap gap-x-6 gap-y-1 text-sm text-slate-500 font-bold">
                                    ...
                                    <div className="mt-2 flex items-center gap-2 text-sm text-slate-600 font-bold">
                                    ...
                                    {app.onDutyNote && (
                                        <div className="mt-2 flex items-center gap-2 text-xs text-indigo-600 font-bold">
