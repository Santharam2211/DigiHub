...
const ManageFaculty = () => {
    ...
    const fetchFaculty = async () => {
        try {
            ...
            const facultyRoles = ['Class Coordinator', 'Program Coordinator', 'Faculty'];
        ...
        <div className="space-y-12 pb-40">
            ...
            <AnimatePresence>
                {showForm && (
                    ...
                    >
                        <h2 className="text-2xl font-black text-slate-900 mb-8">{editingId ? 'Edit Faculty' : 'New Faculty Account'}</h2>
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Password {editingId && '(Leave blank to keep current)'}</label>
                                    <div className="relative">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Employee ID</label>
                                    <div className="relative">
                                ...
                                {form.role === 'Class Coordinator' && (
                                    <>
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Assigned Year</label>
                                            <select
                                                className="input-premium"
                                                value={form.assignedYear} onChange={e => setForm({ ...form, assignedYear: e.target.value })}
                                            >
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Assigned Section</label>
                                            <select
                                                className="input-premium"
                                                value={form.assignedSection} onChange={e => setForm({ ...form, assignedSection: e.target.value })}
                                            >
                            ...
                            <div className="flex justify-end gap-4">
                                <button type="button" onClick={resetForm} className="px-8 py-3 rounded-2xl text-slate-400 font-bold hover:bg-slate-50 transition-all">Cancel</button>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                <div className="px-10 py-8 border-b border-slate-50 space-y-8">
                    <h2 className="text-2xl font-black text-slate-900">Faculty Directory</h2>
                </div>
                    ...
                    {isLoading ? (
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {faculty.map(fac => (
                                    <tr key={fac._id} className="hover:bg-slate-50 transition-colors group">
                                        ...
                                        <td className="px-10 py-6">
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${fac.role === 'Admin' ? 'bg-slate-900 text-white' :
                                                fac.role === 'Program Coordinator' ? 'bg-amber-100 text-amber-600' :
                                                    fac.role === 'Faculty' ? 'bg-violet-100 text-violet-650' :
                                                        'bg-indigo-100 text-indigo-600'
                                                }`}>
                                                <UserCheck className="w-3 h-3" />
                                                {fac.role}
                                                {fac.role === 'Class Coordinator' && ` - ${fac.assignedYear} ${fac.assignedSection}`}
                                            </span>
                                        </td>
                                        <td className="px-10 py-6">
                                            <div className="flex items-center gap-3">
                                                <button
                                                    ...
                                                    className="p-3 text-slate-600 hover:bg-white rounded-xl transition-all shadow-sm border border-slate-100"
                                                    title="Edit Account"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className={`p-3 text-red-600 hover:bg-white rounded-xl transition-all shadow-sm border border-slate-100 ${fac.role === 'Admin' ? 'opacity-50 cursor-not-allowed' : ''}`}
