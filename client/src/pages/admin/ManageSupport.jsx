...
const ManageSupport = () => {
    ...
    return (
        <div className="space-y-12 pb-40">
            <header>
                <h1 className="text-5xl font-black text-slate-900 tracking-tighter">Grievance <span className="text-reveal">Redressal.</span></h1>
                <p className="text-slate-500 font-medium mt-2">Monitor, manage, and resolve user complaints and support tickets.</p>
            </header>
            ...
            <div className="grid grid-cols-1 gap-6">
                <AnimatePresence>
                    {isLoading ? (
                        ...
                        <div className="bg-slate-50 rounded-[2.5rem] p-20 text-center space-y-4">
                        ...
                        filteredQueries.map((q) => (
                            <motion.div 
                                ...
                                className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 grid lg:grid-cols-4 gap-8 items-center"
                            >
                                <div className="lg:col-span-2 space-y-3">
                                    <div className="flex items-center gap-3">
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full">{q.category}</span>
                                        <div className={`px-3 py-1 rounded-full text-[10px] font-black border uppercase tracking-wider ${getStatusStyle(q.status)}`}>
                                            {q.status}
                                        </div>
                                    </div>
                                    <h3 className="text-xl font-black text-slate-900 leading-tight">{q.subject}</h3>
                                    <p className="text-slate-500 text-sm line-clamp-1">{q.message}</p>
                                </div>

                                <div className="space-y-1">
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Submitted By</p>
                                    <p className="font-bold text-slate-900">{q.user?.username}</p>
                        ...
                        >
                            <div className="p-10 space-y-8">
                                <div className="flex justify-between items-start">
                                    <div className="space-y-1">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-indigo-600">Grievance Redressal</span>
                                        <h2 className="text-3xl font-black text-slate-900 leading-tight">Resolution Form</h2>
                                    </div>
                                ...
                                <div className="space-y-4 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-xs font-bold text-slate-900">{selectedQuery.subject}</span>
                                    </div>
                                    <p className="text-slate-600 font-medium py-2 border-t border-slate-200 mt-2">{selectedQuery.message}</p>
                                </div>

                                <form onSubmit={handleUpdate} className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-sm font-black text-slate-400 uppercase tracking-widest pl-1">Update Status</label>
                                        <select 
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-sm font-black text-slate-400 uppercase tracking-widest pl-1">Admin Remarks</label>
                                        <textarea 
