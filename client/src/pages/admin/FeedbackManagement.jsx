...
import {
    ...
    Meh, Frown, Brain, PieChart as PieChartIcon
} from 'lucide-react';
...
const CustomBarTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-white border border-gray-200 shadow-xl rounded-xl px-4 py-3">
                <p className="text-xs font-bold text-gray-500 mb-1">{label}</p>
                <p className="text-lg font-black text-primary-700">{payload[0].value} responses</p>
            </div>
        );
    }
    return null;
...
const FeedbackManagement = () => {
    ...
    const loadFeedback = async (event) => {
        ...
        setFeedbackData([]);
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/feedback/event/${event._id}`);
                ...
                {selectedEvent && (
                    <div className="flex gap-3">
                        <button
                            ...
                            className="bg-white text-gray-700 font-bold px-6 py-3 rounded-2xl border border-gray-200 flex items-center gap-2 hover:bg-gray-50 transition-all shadow-sm disabled:opacity-50"
                        >
                                ...
                                {overallSentiment && (
                                    <>
                                        <div className="bg-emerald-50 p-5 rounded-2xl border border-emerald-100 shadow-sm text-center">
                                            <p className="text-3xl font-black text-emerald-600">{overallSentiment.pos}</p>
                                            <p className="text-xs text-emerald-600 font-bold uppercase tracking-widest mt-1">Positive 😊</p>
                                        </div>
                                        <div className="bg-rose-50 p-5 rounded-2xl border border-rose-100 shadow-sm text-center">
                                            <p className="text-3xl font-black text-rose-600">{overallSentiment.neg}</p>
                                            <p className="text-xs text-rose-600 font-bold uppercase tracking-widest mt-1">Negative 😞</p>
                                        </div>
                                    </>
                                )}
                                {!overallSentiment && (
                                            ...
                                            {info.type === 'text' && (
                                                <div className="space-y-2">
                                                    <div className="flex gap-3 text-xs mb-3">
                                                        <span className="flex items-center gap-1 px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-full font-bold">
                                                            <SmilePlus className="w-3.5 h-3.5" /> Positive: {info.sentimentCount.positive}
                                                        </span>
                                                        <span className="flex items-center gap-1 px-3 py-1.5 bg-amber-100 text-amber-700 rounded-full font-bold">
                                                            <Meh className="w-3.5 h-3.5" /> Neutral: {info.sentimentCount.neutral}
                                                        </span>
                                                        <span className="flex items-center gap-1 px-3 py-1.5 bg-rose-100 text-rose-700 rounded-full font-bold">
