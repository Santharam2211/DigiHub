    ...
    const renderExportButtons = () => {
        ...
        return (
            <div className="flex items-center gap-3 flex-wrap mb-2">
                <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Export All Registrations:</span>
                <button
                    onClick={downloadPDF}
    ...
    const renderDownloadSection = () => {
        ...
        return (
            <div className="flex items-center gap-3 flex-wrap mb-4">
                <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Download Registrations by Event:</span>
                <select
                    value={selectedEventId}
                    onChange={(e) => setSelectedEventId(e.target.value)}
                    className="p-2 rounded-xl border border-slate-200 bg-white text-slate-900 text-sm font-medium"
                >
        ...
        <div className="space-y-12 pb-40">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {cards.map((card, i) => (
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        className="group bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.06)] transition-all duration-500"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="flex flex-wrap justify-between items-center gap-4 px-2">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 outline-none transition-all w-64"
                                value={searchTerm}
                    ...
                    {/* Status Filter Tabs */}
                    <div className="flex gap-2 p-1.5 bg-slate-50 rounded-2xl overflow-x-auto">
                        {statusTabs.map(tab => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap flex items-center gap-2 ${statusFilter === tab
                        ...
                        {isLoading ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
                                {filteredEvents.map((event) => (
                                    <div key={event._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all p-6 group flex flex-col justify-between">
                                        <div>
                                            <div className="flex justify-between items-start mb-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center text-slate-500 group-hover:bg-indigo-600 group-hover:text-white transition-all shrink-0">
                                                        <Calendar className="w-6 h-6" />
                                                    </div>
                                                    <div>
                                                        <h3 className="font-black text-slate-900 group-hover:text-indigo-600 transition-colors line-clamp-1">{event.title}</h3>
                                                        <span className="px-2.5 py-1 bg-slate-50 text-slate-600 rounded-lg text-[10px] font-bold border border-slate-100 inline-block mt-1">
                                                            {event.category}
                                                        </span>
                                                    </div>
                                                </div>
                                        ...
                                        <div className="pt-4 border-t border-slate-100 flex items-center justify-end gap-2 mt-auto">
                                            <Link
                                                to={`/events/${event._id}`}
                                                className="p-2.5 bg-slate-50 hover:bg-slate-100 rounded-xl transition-all text-slate-500 hover:text-indigo-600 flex items-center gap-2 text-xs font-bold"
                                                title="View Event"
                                            >
                                            ...
                                            <Link
                                                ...
                                                className="p-2.5 bg-slate-50 hover:bg-indigo-50 rounded-xl transition-all text-slate-500 hover:text-indigo-600 flex items-center gap-2 text-xs font-bold"
                                                title="Edit Event"
                                            >
                                            ...
                                            <button
                                                ...
                                                className="p-2.5 bg-slate-50 hover:bg-red-50 rounded-xl transition-all text-slate-500 hover:text-red-500 flex items-center gap-2 text-xs font-bold"
                                                title="Delete Event"
                                            >
                                            ...
                                            <button
                                                ...
                                                className="p-2.5 bg-slate-50 hover:bg-emerald-50 rounded-xl transition-all text-slate-500 hover:text-emerald-600 flex items-center gap-2 text-xs font-bold"
                ...
                <div className="space-y-8">
                    <h2 className="text-2xl font-black text-slate-900 px-2">Instant Actions</h2>
                    <div className="grid gap-4">
                        {[
                            ...
                            <Link key={i} to={action.link} className="group bg-white p-6 rounded-[2rem] border border-slate-100 flex items-center justify-between hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300">
                                <div className="flex items-center gap-5">
                                    <div className={`w-12 h-12 rounded-xl ${action.color} flex items-center justify-center shadow-sm`}>
                                        {action.icon}
                                    </div>
                                    <span className="text-base font-black text-slate-900">{action.title}</span>
                                </div>
                    ...
                    <div className="bg-indigo-600 p-10 rounded-[3rem] text-white relative overflow-hidden shadow-2xl">
                        ...
                        <div className="relative z-10 space-y-5">
                            ...
                            <button className="px-6 py-2.5 bg-white text-indigo-600 rounded-xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all">
                                Diagnostics
                            </button>
                        </div>
                    </div>
