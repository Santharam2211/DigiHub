...
const AttendanceRecords = () => {
    ...
    useEffect(() => {
        ...
        const fetchRecords = async () => {
            setIsLoading(true);
            try {
                const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/attendance/records/${selectedEvent}`);
                setRecords(res.data);
            } catch (error) {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            try {
                const settingsRes = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/settings`);
                if (settingsRes.data) {
            ...
            const backendUrl = import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}`;
                                ...
                                {filteredRecords.length === 0 ? (
                                    ...
                                    filteredRecords.map((reg, index) => (
                                        <tr key={reg._id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-8 py-5 text-sm font-medium text-slate-600">{reg.participant?.yearAndDept || '-'}</td>
                                            <td className="px-8 py-5 text-sm font-medium text-slate-600">{reg.participant?.section || '-'}</td>
