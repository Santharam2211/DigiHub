        ...
        <div className="space-y-8 pb-40">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-wrap justify-between items-center gap-6">
                <div className="flex items-center gap-5">
                    <div className="w-16 h-16 bg-primary-100 rounded-3xl flex items-center justify-center text-primary-700">
                        <Award className="w-8 h-8" />
                    </div>
                    <div>
                        <h1 className="text-4xl font-black text-slate-900 tracking-tight">Nomination Review</h1>
                        <p className="text-slate-500 font-medium italic">Multi-stage approval workflow</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                        ...
                        <AnimatePresence>
                            {showExportMenu && (
                                <motion.div
                                    ...
                                    className="absolute right-0 top-full mt-2 w-52 bg-white border border-slate-100 rounded-2xl shadow-xl shadow-slate-200/60 z-50 overflow-hidden"
                                >
                                    <div className="p-2 space-y-1">
                                        ...
                                        >
                                            ...
                                            <div>
                                                <p className="font-bold text-sm text-slate-900 group-hover:text-red-700">Export as PDF</p>
                                        ...
                                        >
                                            ...
                                            <div>
                                                <p className="font-bold text-sm text-slate-900 group-hover:text-emerald-700">Export as XLSX</p>
                    ...
                    >
                        ...
                        <option value="Pending Class Coordinator">Class Coordinator</option>
                        <option value="Pending Program Coordinator">Program Coordinator</option>
                                ...
                                >
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${
                                            n.status.includes('Approved') ? 'bg-emerald-500/20 text-emerald-500' :
                                            n.status.includes('Rejected') ? 'bg-red-500/20 text-red-500' :
                    ...
                    <AnimatePresence mode="wait">
                        {!selectedNomination ? (
                            <motion.div 
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="bg-white rounded-[3rem] border border-slate-100 p-24 text-center space-y-6"
                            >
                                ...
                                <div className="p-10 space-y-10">
                                                    ...
                                                    {!settings.disabledDefaultFields?.includes('name') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-slate-900 font-black">{selectedNomination.personalInfo?.name || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                    {!settings.disabledDefaultFields?.includes('rollNumber') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-slate-900 font-black">{selectedNomination.personalInfo?.rollNumber || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                    {!settings.disabledDefaultFields?.includes('gender') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-slate-900 font-black">{selectedNomination.personalInfo?.gender || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                    {!settings.disabledDefaultFields?.includes('dateOfBirth') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-slate-900 font-black">{selectedNomination.personalInfo?.dateOfBirth || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                    {!settings.disabledDefaultFields?.includes('year') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-slate-900 font-black">{selectedNomination.personalInfo?.year || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                    {!settings.disabledDefaultFields?.includes('section') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-slate-900 font-black">{selectedNomination.personalInfo?.section || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                </div>
                                                    ...
                                                    {!settings.disabledDefaultFields?.includes('tenthPercentage') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-primary-600 font-black">{selectedNomination.academicProficiency?.tenthPercentage || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                    {!settings.disabledDefaultFields?.includes('diplomaPercentage') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-primary-600 font-black">{selectedNomination.academicProficiency?.diplomaPercentage || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                    {!settings.disabledDefaultFields?.includes('cgpa') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-primary-600 font-black">{selectedNomination.academicProficiency?.cgpa || 'N/A'}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                    {!settings.disabledDefaultFields?.includes('noOfArrears') && (
                                                        <div className="flex justify-between items-center text-sm gap-2">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <span className="text-emerald-600 font-black">{selectedNomination.academicProficiency?.noOfArrears || 0}</span>
                                                            )}
                                                        </div>
                                                    )}
                                                </div>
                                        ...
                                        <div className="space-y-4">
                                            {selectedNomination.previousPositions?.length > 0 ? (
                                                selectedNomination.previousPositions.map((pos, idx) => (
                                                    <div key={idx} className="bg-white p-4 rounded-2xl border border-slate-200 grid md:grid-cols-3 gap-4">
                                                        <div>
                                                            <span className="block text-[10px] text-slate-400 font-black uppercase tracking-widest">Body</span>
                                                            <span className="text-sm font-bold text-slate-900">{pos.nameOfBody}</span>
                                                        </div>
                                                        <div>
                                                            <span className="block text-[10px] text-slate-400 font-black uppercase tracking-widest">Position</span>
                                                            <span className="text-sm font-bold text-slate-900">{pos.position}</span>
                                                        </div>
                                                        <div>
                                                            <span className="block text-[10px] text-slate-400 font-black uppercase tracking-widest">Period</span>
                                                            <span className="text-sm font-bold text-slate-900">{pos.period}</span>
                                                        </div>
                                                    </div>
                                                ))
                                            ) : (
                                                <p className="text-slate-500 italic text-sm">No previous positions declared.</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-slate-50 rounded-[2rem] p-8 space-y-6 border border-slate-100">
                                        ...
                                        <div className="grid md:grid-cols-2 gap-6">
                                            <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-2">
                                                <span className="block text-xs text-primary-600 font-black uppercase tracking-widest">1. Academic</span>
                                                {isEditing ? (
                                                    <textarea 
                                                    ...
                                                    <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{selectedNomination.contributions?.academic || 'N/A'}</p>
                                                )}
                                            </div>
                                            <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-2">
                                                <span className="block text-xs text-primary-600 font-black uppercase tracking-widest">2. Co-Curricular</span>
                                                {isEditing ? (
                                                    <textarea 
                                                        className="input-premium w-full min-h-[100px]"
                                                    ...
                                                    <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{selectedNomination.contributions?.coCurricular || 'N/A'}</p>
                                                )}
                                            </div>
                                            <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-2">
                                                <span className="block text-xs text-primary-600 font-black uppercase tracking-widest">3. Extracurricular</span>
                                                {isEditing ? (
                                                    <textarea 
                                                        className="input-premium w-full min-h-[100px]"
                                                    ...
                                                    <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{selectedNomination.contributions?.extracurricular || 'N/A'}</p>
                                                )}
                                            </div>
                                            <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-2">
                                                <span className="block text-xs text-primary-600 font-black uppercase tracking-widest">4. Notable Contributions</span>
                                                {isEditing ? (
                                                    <textarea 
                                                        className="input-premium w-full min-h-[100px]"
                                                    ...
                                                    <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{selectedNomination.contributions?.otherNotable || 'N/A'}</p>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="space-y-4">
                                            {selectedNomination.approvalHistory.length === 0 ? (
                                                <p className="text-sm text-slate-400 pl-4 py-4 border-l-2 border-dashed border-slate-200">No approval history yet.</p>
                                            ) : (
                                                selectedNomination.approvalHistory.map((history, i) => (
                                                    <div key={i} className="flex gap-6 relative">
                                                        ...
                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black ${
                                                            history.status === 'Approved' ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'
                                                        }`}>
                                                            {history.status === 'Approved' ? <CheckCircle className="w-6 h-6" /> : <XCircle className="w-6 h-6" />}
                                                        </div>
                                                        <div className="flex-1 bg-white p-6 rounded-3xl border border-slate-50 shadow-sm">
                                                            <div className="flex justify-between items-start">
                                                                <div>
                                                                    <p className="font-black text-slate-900">{history.stage}</p>
                                                            ...
                                                            <p className="mt-3 text-sm text-slate-600 font-medium bg-slate-50 p-4 rounded-2xl italic">"{history.remarks || 'No remarks provided.'}"</p>
                                                        </div>
                                                    </div>
                                                ))
                                            )}
                                    ...
                                    {selectedNomination.status !== 'Approved' && selectedNomination.status !== 'Rejected' && (
                                        ...
                                        <div className="pt-10 border-t border-slate-100 space-y-6">
                                            <h3 className="text-xl font-black text-slate-900">Your Decision</h3>
                                            <textarea 
